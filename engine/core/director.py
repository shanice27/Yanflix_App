...
def build_user_prompt(segments: list, show_name: str = "", episode_context: str = "", cast_map: Optional[dict] = None) -> str:
    context_block = ""
    if show_name:
...
def _llm_generate(prompt: str, retries: int) -> str:
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                wait = 2 ** attempt
                print(f"[Director] Attempt {attempt+1} failed: {e}. Retrying in {wait}s...")
                time.sleep(wait)
            else:
                raise
...
def apply_emotion_tags(
    ...
    for chunk_num, chunk in enumerate(chunks):
        ...
        for attempt in range(retries):
            try:
                raw = _llm_generate(prompt, retries=1).strip()
                if raw.startswith("```"):
                    raw = raw.split("```")[1]
